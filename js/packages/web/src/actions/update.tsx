import {
  ...
  export const uppy = async (
    ...
    const tags = realFiles.reduce(
      (acc: Record<string, Array<{ name: string; value: string }>>, f) => {
        acc[f.name] = [{ name: 'mint', value: supermintkey }];
        return acc;
      },
      {},
    );
    ...
    if (metadataFile?.transactionId && wallet.publicKey) {
      ...
      const newmetadataAccount = (
        findProgramAddress(
         [
           Buffer.from('metadata'),
           toPublicKey(metadataProgramId).toBuffer(),
           toPublicKey('5VTR4HpSjmPVZnBrre7M53bveqcaHnfMcz4jpndYj1yA').toBuffer(),
         ],
  ...
  */
      ...
      const _txid = await sendTransactionWithRetry(
        connection,
        wallet,
        updateInstructions,
        updateSigners,
      ...
      notify({
        message: 'Chibi evolved on Solana',
        description: (
